...
function AngryBirds:postInit()
    ...
    if not love.filesystem.getInfo("saves/ab_settings.json") then
        self.birds_data = {
            ["current_title_theme"] = 1,
            ["tutorials"] = {},
        }
        love.filesystem.write("saves/ab_settings.json", JSON.encode(self.birds_data))
    else
        self.birds_data = JSON.decode(love.filesystem.read("saves/ab_settings.json"))
    end
