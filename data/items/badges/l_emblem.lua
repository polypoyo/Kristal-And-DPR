...
function badge:update(equipped)
	if equipped and Game.battle and Game.battle.party and #Game.battle.party > 0 and (not Game.battle.party[1].emblem) and (BadgesLib:getBadgeEquipped("w_emblem") <= 0) then
		Game.battle.party[1].emblem = ColorMaskFX({0,1,0}, 0.5)
...
function badge:onBadgeEquipped()
	if (BadgesLib:getBadgeEquipped("w_emblem") > 0) then
		Kristal.Console:log("W Emblem detected")
	end
